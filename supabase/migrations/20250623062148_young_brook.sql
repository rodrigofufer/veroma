/*
  # Complete Database Schema Setup with Policy Conflict Fix

  1. New Tables
    - `profiles` - User profiles with roles and moderation
    - `ideas` - User-generated content (proposals, complaints, votes)
    - `votes` - User votes on ideas
    - `comments` - Comments on ideas with moderation
    - `notifications` - System notifications
    - `audit_log` - Comprehensive audit trail
    - `admin_audit_log` - Administrative actions audit
    - `error_codes` - Error codes and messages
  ...
  3. Functions
    ...
    - Voting system with weekly limits
...
-- Drop existing policies on ideas
...
-- Drop existing policies on votes
...
-- Drop existing policies on admin_audit_log
DROP POLICY IF EXISTS "Admins can view audit log" ON public.admin_audit_log;

-- Drop existing policies on error_codes
