/*
  ...
  This migration creates the entire database schema from scratch, replacing all previous
  migrations with a single clean implementation.

  1. Tables
    - profiles - User profiles with roles and voting information
    - ideas - Core content (proposals, complaints, votes)
...
-- Drop functions
DROP FUNCTION IF EXISTS public.handle_anonymous_idea();
...
DROP FUNCTION IF EXISTS public.sync_user_email_confirmation(uuid);
DROP FUNCTION IF EXISTS public.is_user_email_verified(uuid);
DROP FUNCTION IF EXISTS public.force_email_verification(uuid);
DROP FUNCTION IF EXISTS public.debug_auth_status(text);
DROP FUNCTION IF EXISTS public.fix_user_login(text);
DROP FUNCTION IF EXISTS public.check_user_verified_and_not_banned();
DROP FUNCTION IF EXISTS public.is_admin_or_authority();
DROP FUNCTION IF EXISTS public.can_create_official_proposals();
DROP FUNCTION IF EXISTS public.can_modify_official_proposals();
DROP FUNCTION IF EXISTS public.get_user_role();
DROP FUNCTION IF EXISTS public.create_audit_log_entry(uuid, text, text, uuid, jsonb);
DROP FUNCTION IF EXISTS public.get_current_actor_id();
DROP FUNCTION IF EXISTS public.audit_ideas_changes();
DROP FUNCTION IF EXISTS public.audit_profiles_changes();
DROP FUNCTION IF EXISTS public.audit_votes_changes();
DROP FUNCTION IF EXISTS public.get_audit_logs(text, uuid, uuid, text, integer, integer);
...
CREATE POLICY "Users can update their own ideas"
  ON ideas FOR UPDATE
  TO authenticated
  USING ((uid() = user_id) AND check_user_verified_and_not_banned())
