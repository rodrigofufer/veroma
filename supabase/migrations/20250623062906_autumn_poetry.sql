/*
  # Fix database schema with policy conflict resolution
  
  This migration creates the core database schema for the Veroma platform, including:
  ...
  1. Tables
    ...
    - admin_audit_log: Administrative action logs
    - error_codes: Error message definitions
  ...
  This migration drops existing triggers and policies first to avoid conflicts.
*/
...
DROP POLICY IF EXISTS "Profiles are viewable by everyone including anonymous users" ON public.profiles;
...
DROP POLICY IF EXISTS "Representatives can update official proposals" ON public.ideas;
...
DROP POLICY IF EXISTS "Admins can manage any vote" ON public.votes;
...
DROP POLICY IF EXISTS "Admins can manage any comment" ON public.comments;
...
-- Drop policies for audit_log table
DROP POLICY IF EXISTS "Admins and authorities can view audit log" ON public.audit_log;
DROP POLICY IF EXISTS "Service role has full access to audit log" ON public.audit_log;

-- Drop policies for admin_audit_log table
DROP POLICY IF EXISTS "Admins can view audit log" ON public.admin_audit_log;

-- Drop policies for error_codes table
...
DROP TRIGGER IF EXISTS handle_anonymous_idea_trigger ON public.ideas;
...
CREATE TRIGGER handle_anonymous_idea_trigger
  ...
  FOR EACH ROW EXECUTE FUNCTION public.handle_anonymous_idea();
