-- ============================================================================
-- DROP EXISTING SCHEMA (if rebuilding an existing database)
-- ============================================================================

-- First drop policies that depend on functions to avoid dependency errors
DO $$ 
BEGIN
  -- Drop all RLS policies to avoid dependency issues
  EXECUTE 'DROP POLICY IF EXISTS "Representatives can update official proposals" ON ideas';
  EXECUTE 'DROP POLICY IF EXISTS "Users can create their own proposals" ON proposals';
  EXECUTE 'DROP POLICY IF EXISTS "Users can update their own proposals" ON proposals';
  EXECUTE 'DROP POLICY IF EXISTS "Users can create their own ideas" ON ideas';
  EXECUTE 'DROP POLICY IF EXISTS "Users can vote once per idea" ON votes';
  EXECUTE 'DROP POLICY IF EXISTS "Users can update their own ideas" ON ideas';
  EXECUTE 'DROP POLICY IF EXISTS "Users can delete their own ideas" ON ideas';
  EXECUTE 'DROP POLICY IF EXISTS "Users can update their own votes" ON votes';
  EXECUTE 'DROP POLICY IF EXISTS "Users can delete their own votes" ON votes';
  ...
  EXECUTE 'DROP POLICY IF EXISTS "Admins can update any profile" ON profiles';
  EXECUTE 'DROP POLICY IF EXISTS "Admins can delete any profile" ON profiles';
  EXECUTE 'DROP POLICY IF EXISTS "Admins can update any idea" ON ideas';
  EXECUTE 'DROP POLICY IF EXISTS "Admins can delete any idea" ON ideas';
  EXECUTE 'DROP POLICY IF EXISTS "Admins can manage any vote" ON votes';
  EXECUTE 'DROP POLICY IF EXISTS "Admins can manage any comment" ON comments';
  EXECUTE 'DROP POLICY IF EXISTS "Admins and authorities can view audit log" ON audit_log';
  EXECUTE 'DROP POLICY IF EXISTS "Admins can view audit log" ON admin_audit_log';
  EXECUTE 'DROP POLICY IF EXISTS "Admins can manage notifications" ON notifications';
EXCEPTION
  WHEN OTHERS THEN
...
-- Drop triggers (except those on auth.users, which we don't have permission for)
DROP TRIGGER IF EXISTS handle_anonymous_idea_trigger ON ideas;
DROP TRIGGER IF EXISTS on_vote_changed ON votes;
DROP TRIGGER IF EXISTS audit_ideas_trigger ON ideas;
DROP TRIGGER IF EXISTS audit_profiles_trigger ON profiles;
DROP TRIGGER IF EXISTS audit_votes_trigger ON votes;
-- Note: We skip dropping the on_auth_user_created trigger on auth.users as we don't have permissions

-- Drop functions with CASCADE option to handle dependencies
DROP FUNCTION IF EXISTS public.handle_anonymous_idea() CASCADE;
DROP FUNCTION IF EXISTS public.update_vote_counts() CASCADE;
DROP FUNCTION IF EXISTS public.handle_new_user() CASCADE;
DROP FUNCTION IF EXISTS public.increment_vote(uuid, text) CASCADE;
DROP FUNCTION IF EXISTS public.get_user_vote_history(uuid) CASCADE;
DROP FUNCTION IF EXISTS public.sync_user_email_confirmation(uuid) CASCADE;
DROP FUNCTION IF EXISTS public.is_user_email_verified(uuid) CASCADE;
DROP FUNCTION IF EXISTS public.force_email_verification(uuid) CASCADE;
DROP FUNCTION IF EXISTS public.debug_auth_status(text) CASCADE;
DROP FUNCTION IF EXISTS public.fix_user_login(text) CASCADE;
DROP FUNCTION IF EXISTS public.check_user_verified_and_not_banned() CASCADE;
DROP FUNCTION IF EXISTS public.is_admin_or_authority() CASCADE;
DROP FUNCTION IF EXISTS public.can_create_official_proposals() CASCADE;
DROP FUNCTION IF EXISTS public.can_modify_official_proposals() CASCADE;
DROP FUNCTION IF EXISTS public.get_user_role() CASCADE;
DROP FUNCTION IF EXISTS public.create_audit_log_entry(uuid, text, text, uuid, jsonb) CASCADE;
DROP FUNCTION IF EXISTS public.get_current_actor_id() CASCADE;
DROP FUNCTION IF EXISTS public.audit_ideas_changes() CASCADE;
DROP FUNCTION IF EXISTS public.audit_profiles_changes() CASCADE;
DROP FUNCTION IF EXISTS public.audit_votes_changes() CASCADE;
DROP FUNCTION IF EXISTS public.get_audit_logs(text, uuid, uuid, text, integer, integer) CASCADE;
DROP FUNCTION IF EXISTS public.get_audit_summary() CASCADE;
DROP FUNCTION IF EXISTS public.auto_confirm_email() CASCADE;
DROP FUNCTION IF EXISTS public.log_admin_action(uuid, text, jsonb) CASCADE;
DROP FUNCTION IF EXISTS public.admin_ban_user(uuid, boolean, text) CASCADE;
...
DROP FUNCTION IF EXISTS public.assign_strike(uuid) CASCADE;
DROP FUNCTION IF EXISTS public.send_system_notification(uuid, text) CASCADE;
...
DROP TABLE IF EXISTS public.error_codes CASCADE;
...
CREATE INDEX idx_ideas_country ON public.ideas (country);
CREATE INDEX idx_ideas_category ON public.ideas (category);
CREATE INDEX idx_ideas_type ON public.ideas (type);
CREATE INDEX idx_ideas_location_level ON public.ideas (location_level, location_value);
...
-- Votes indexes
CREATE INDEX idx_votes_idea_user ON public.votes (idea_id, user_id);
...
CREATE INDEX idx_comments_is_flagged ON public.comments (is_flagged);
...
CREATE INDEX idx_audit_log_entity_type ON public.audit_log (target_entity_type);
CREATE INDEX idx_audit_log_entity_id ON public.audit_log (target_entity_id);
...
-- Note: We don't create the trigger on auth.users directly
-- Instead, we'll use Supabase's built-in hooks to create users
...
-- Note: We don't create the trigger for auto_confirm_email directly
-- For development environments, this would need to be enabled through Supabase's hooks
